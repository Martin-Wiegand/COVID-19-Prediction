...
# modified from GPL code https://cran.r-project.org/web/packages/crrp/index.html
# modified to be faster for our use-case

gcrrp_mod = function (time, fstatus, X, failcode = 1, cencode = 0, group = 1:ncol(X), 
                      penalty = c("gLASSO", "gMCP", "gSCAD"), 
                      gamma = switch(penalty, SCAD = 3.7, 2.7), alpha = 1, lambda.min = 0.001, 
                      nlambda = 50, lambda, eps = 0.001, max.iter = 1000, weighted = FALSE) 
{
  norm <- function(beta) {
  ...
  dgpenalty <- function(beta, K, gamma = switch(penalty, SCAD = 3.7, 
                                                2.7), lambda, penalty = c("gLASSO", "gSCAD", 
                                                                          "gMCP"), penalty.factor) {
    ...
    if (penalty == "gLASSO") {
      ans[norm(beta) > 0] = (norm(beta[norm(beta) > 0]))^{
        -1
      }
      ans[norm(beta) == 0] = 0
      return(ans * lambda)
      ...
      ans[norm(beta) > 0] = (1 * (norm(beta[abs(beta) > 
                                              0]) <= lambda) * lambda + pmax(gamma * lambda - 
                                                                               norm(beta[abs(beta) > 0]), 0)/(gamma - 1) * (norm(beta[abs(beta) > 
                                                                                                                                        0]) > lambda))/norm(beta[abs(beta) > 0])
      ans[beta == 0] = 0
      return(ans)
    }
    else if (penalty == "gMCP") {
      ans[norm(beta) > 0] = 1 * (norm(beta[norm(beta) > 
                                             0]) <= gamma * lambda) * (lambda - norm(beta[norm(beta) > 
                                                                                            0])/gamma)/norm(beta[norm(beta) > 0])
      ans[norm(beta) == 0] = 0
      return(ans)
    }
  }
  ...
  if (length(group) != ncol(X)) 
    stop("group vector and number of columns of X must be the same")
  if (gamma <= 1 & penalty == "gMCP") 
    stop("gamma must be greater than 1 for the MC penalty")
  if (gamma <= 2 & penalty == "gSCAD") 
  ...
  X <- d$X
  u <- do.call("survfit", list(formula = Surv(ftime, 
                                              cenind) ~ 1, data = data.frame(ftime, cenind)))
  u <- approx(c(0, u$time, max(u$time) * (1 + 10 * .Machine$double.eps)), 
              c(1, u$surv, 0), xout = ftime * (1 - 100 * .Machine$double.eps), 
              method = "constant", f = 0, rule = 2,ties = mean) # ties argument added in to avoid "collapsing x" warning 
  uuu <- u$y
  ...
  if (weighted) {
    ...
    for (jj in 1:(length(size) - 1)) {
      weight <- c(weight, sqrt(size[jj + 1] - size[jj])/norm(init[(size[jj] + 
                                                                     1):size[jj + 1]]))
    }
  ...
  if (missing(lambda)) {
    ...
    l.min <- lambda.min
    lambda = seq(l.max,l.min * l.max,len = nlambda)
    # browser()
    # lambda = exp(seq(log(l.max), log(l.min * l.max), len = nlambda))
    # lambda = log(seq(exp(l.max), exp(l.min * l.max), len = nlambda))
    user.lambda <- FALSE
  }
  else {
  ...
  res <- .Call("gcdfit_psh", XX, as.numeric(ftime), as.integer(fstatus), 
               uuu, K1, penalty, lambda, eps, as.integer(max.iter), 
               gamma, as.numeric(penalty.factor), alpha, PACKAGE = "crrp")
  ...
  #      pp <- c(pp, dgpenalty(beta = b[(K1[k] + 1):K1[k + 
  #                                                      1], l], K[k], lambda = lambda[l], penalty = penalty, 
  #                            penalty.factor = penalty.factor[k]))
  #    }
     # ps[l] <- sum(diag(XX %*% ginv(zwz + n * diag(pp)) %*% 
    #                      t(XX) %*% diag(hessian[, l]))) - sum(b[, l] == 0)
  ...
  val <- structure(list(beta = beta, iter = iter, group = group, 
                        lambda = lambda, penalty = penalty, gamma = gamma, 
                        alpha = alpha,BIC = bic), class = "gcrrp")
  val
}
